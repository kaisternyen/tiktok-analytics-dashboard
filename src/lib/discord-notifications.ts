...
import { getNewPostMessage } from './phase-tracking';
...
export async function sendDiscordNotification(message: string, channel: 'viral-alerts' | 'new-posts' = 'viral-alerts'): Promise<boolean> {
  ...
  try {
    ...
    if (response.ok) {
      console.log(`✅ Discord notification sent successfully to ${channel} channel`);
      return true;
    } else {
...
export async function notifyNewVideo(
  ...
  const message = getNewPostMessage(
    username,
    platform,
    url,
    getDescription(mediaData),
    getViews(mediaData),
