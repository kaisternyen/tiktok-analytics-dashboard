...
export async function GET() {
    try {
        ...
        // For each account, fetch trackedPosts only (fast database query)
        const accountsWithCounts = await Promise.all(accounts.map(async (account) => {
            let trackedPosts = 0;
            
            try {
                // Only fetch tracked posts count - this is fast
                ...
                console.error('Error fetching tracked posts for account', account.username, account.platform, err);
            }
            
            // Return basic account info without external API calls for fast loading
            return {
                id: account.id,
                username: account.username,
                platform: account.platform,
                ...
                trackedPosts,
                // Use placeholder values for now - these can be populated during account checks
                totalPosts: null,
                pfpUrl: null,
                apiStatus: null,
                apiError: null,
                ...
                followers: null,
                profileUrl: account.platform === 'tiktok' ? `https://www.tiktok.com/@${account.username}` :
                          account.platform === 'instagram' ? `https://www.instagram.com/${account.username}/` :
                          account.platform === 'youtube' ? `https://www.youtube.com/@${account.username}` : null,
